...
def model_repo(model_repo_path: str, artifact_uri: str) -> tsbk.TritonModelRepo:
    return tsbk.TritonModelRepo(
        ...
        models={
            'addsub': tsbk.TritonModel(
                versions=[
                    tsbk.TritonModelVersion(artifact_uri=artifact_uri)
                ]
            )
        }
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--build-only', action='store_true', help='Only build the model repository without starting the server')
