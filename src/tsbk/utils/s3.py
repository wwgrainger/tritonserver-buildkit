...
def compute_cache_path(s3_path: str, cache_bust: str | None = None) -> Path:
    ...
    Args:
        ...
        cache_bust: optional value used to invalidate the cached object
    ...
    cache_key = append_cache_bust(f"{key}.{etag}", cache_bust)
    return TSBK_DIR / "cache" / "s3" / bucket / cache_key


def download_s3_file(origin_path: str, dst_path: Union[str, Path], cache_bust: str | None = None):
...
def download_s3_path(origin_path: str, dst_path: Union[str, Path], cache_bust: str | None = None, **kwargs):
    ...
    Args:
        ...
        cache_bust: optional value used to invalidate the cached objects
    """
    ...
    for obj in bucket.objects.filter(Prefix=prefix):
        ...
        # Download the file
        download_s3_file(f"s3://{bucket_name}/{obj.key}", obj_local_path, cache_bust=cache_bust)
